    ...
    <div className="grid gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {sorted.map((q) => {
          ...
          const uploadStatus = String(q?.uploadStatus || '')
          const uploadError = String(q?.uploadError || '')
          return (
            <div key={id} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
              ...
              {!q?.imageUrl && uploadStatus === 'pending' ? (
                <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                  Upload in progress… use Refresh in a moment.
                </div>
              ) : null}

              {uploadStatus === 'failed' ? (
                <div className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">
                  Upload failed{uploadError ? `: ${uploadError}` : '.'}
                </div>
              ) : null}
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-800">
                  ...
                  <button
                    ...
                    disabled={submitting || uploadStatus === 'pending' || uploadStatus === 'failed' || !q?.imageUrl}
                    className={
