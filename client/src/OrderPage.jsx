...
function OrderPage({
  ...
  // draft + setDraft are owned by App so login can persist/restore them.
  ...
  useEffect(() => {
    ...
    setDraft((prev) => {
      ...
      for (const key of Object.keys(DEFAULT_DRAFT)) {
        next[key] =
          typeof next[key] === "string" ? next[key] : String(next[key] ?? "");
      }
      return next;
    });
  }, [draft, setDraft]);
  ...
  const clearPaymentScreenshot = () => {
    ...
    setErrors((prev) => {
      if (!prev || typeof prev !== "object" || !("paymentScreenshot" in prev))
        return prev;
      const next = { ...prev };
  ...
  const validateStep1 = () => {
    ...
    else if (!NAME_REGEX.test(leaderName))
      nextErrors.leaderName = "Only letters and spaces are allowed";
    ...
    if (!phone) nextErrors.phone = "Phone number is required";
      ...
      nextErrors.phone = `Phone number must be exactly ${PHONE_LEN} digits`;

    const email = String(draft?.email ?? "").trim();
    if (!email) nextErrors.email = "Email is required";
    else if (!isValidEmail(email)) nextErrors.email = "Enter a valid email";
    else if (!email.toLowerCase().endsWith(KLU_EMAIL_DOMAIN))
      nextErrors.email = `Email must end with ${KLU_EMAIL_DOMAIN}`;

    if (cartItems.length === 0) nextErrors.cart = "Your cart is empty";
    else if (cartItems.some((it) => it?.isActive === false))
      nextErrors.cart = "Remove inactive items from cart to continue";
    else if (Number(totalItems) > 10)
      nextErrors.cart = "Maximum 10 total items allowed per order";
    return nextErrors;
  };
  ...
  const validateStep2 = () => {
    ...
    else if (!TXN_REGEX.test(tid))
      nextErrors.transactionId =
        "Transaction ID must be alphanumeric (A-Z, 0-9) with no spaces";
    else if (txnCheck.status === "used")
      nextErrors.transactionId = "This transaction ID has already been used";
    if (!paymentScreenshotFile)
      nextErrors.paymentScreenshot = "Upload payment screenshot";
    return nextErrors;
  };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setTxnCheck({
          status: "error",
          message: e?.message || "Could not verify transaction ID right now.",
        });
      }
    }, 450);
    ...
    <div className="mx-auto max-w-3xl">
      <div className="mb-4">
        <h1 className="text-lg font-semibold">
          {step === 1 ? "Team & Order Details" : "Payment"}
        </h1>
      ...
      <section
        className="w-full rounded-2xl bg-[#FDE68A]"
                ...
                <div className="grid gap-3">
                  <label className="grid gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDraft((p) => ({ ...p, teamName: e.target.value }))
                      }
                  ...
                  <label className="grid gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDraft((p) => ({ ...p, leaderName: e.target.value }))
                      }
              ...
              >
                ...
                <div className="grid gap-4">
                    ...
                    <div className="mt-2 grid gap-3 sm:grid-cols-[160px_1fr] sm:items-start">
                      ...
                      <div className="text-sm text-slate-700">
                        <div className="font-medium">
                          Pay the amount shown in your Order Summary.
                        </div>
                        <div className="mt-1 text-xs text-slate-600">
                          After payment, upload the screenshot and enter the
                          transaction ID.
                        </div>
                      </div>
                    </div>
                  ...
                  <label className="grid gap-1">
                    ...
                    {txnCheck.status === "invalid" ||
                    txnCheck.status === "used" ||
                    txnCheck.status === "error" ? (
                      <span className="text-xs text-red-600">
                        {txnCheck.message}
                      </span>
                    ) : null}
